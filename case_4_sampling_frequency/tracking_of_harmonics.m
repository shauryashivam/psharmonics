...
%--Normal variables---%
f=50;                       % frequency
fs=2*9*50;                 % sampling frequency should be atleast twice the highest frequency of signal to be analysed
T_cycle = 1/f;             % time period for one cycle
T_max=3*T_cycle;            % max time period
...
R       = 0.1;           %Measurement white noise covariance matrix
...
[K_k] = calculate_k(F,H_k,Q,R,P_0_minus,fs,f,T_vector);
...
%Z = Z+0.05*wgn(1,length(Z),1);
%----Estimate the states using the kalman filtering algorithm-----%
[X_k] = calculate_states(F,H_k,Z,K_k,T_vector,X_0_minus,fs,f);
